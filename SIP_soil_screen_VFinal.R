...
max_locations <- fd_summary %>%
  filter(count.results.gt.screening.level > 0, detect.flag == 'Y') %>%
...
save_soil_table <- function(sma){
  ...
  if (nrow(tmp) > 0){
    ...
    if (nrow(tmp) >= 5) {
    bmp(paste0('Output/', sma, '/', sma, '_soil_exceedance_table', '.bmp'), height = 30*nrow(tmp), width = 200*ncol(tmp))
    table <- tableGrob(tmp)
    ...
    dev.off()
      ...
      bmp(paste0('Output/', sma, '/', sma, '_soil_exceedance_table', '.bmp'), height = 80*nrow(tmp), width = 200*ncol(tmp))
      table <- tableGrob(tmp)
      title <- textGrob(sma, gp = gpar(fontsize = 24))
      padding <- unit(0.5, 'line')
      table <- gtable_add_rows(table, heights = grobHeight(title) + padding, pos = 0)
